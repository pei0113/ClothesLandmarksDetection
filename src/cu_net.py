...
class _Bn_Rulu_Conv_tanh(nn.Sequential):
    def __init__(self, in_num, out_num):
        super(_Bn_Rulu_Conv_tanh, self).__init__()
        self.add_module('norm', nn.BatchNorm2d(in_num))
        self.add_module('relu', nn.ReLU(inplace=True))
        self.add_module('conv', nn.Conv2d(in_num, out_num, kernel_size=1, stride=1, bias=False))
...
class _CU_Net_Wrapper(nn.Module):
    def __init__(self, init_chan_num, neck_size, growth_rate,
        ...
        self.out_layer = _Bn_Rulu_Conv_tanh(in_num=num_chans, out_num=class_num)
